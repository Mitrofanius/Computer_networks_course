...
TARGET_ADDR = '192.168.30.32'
TARGET_PORT = 14001

# PACK_NUM_FRONTEND = 3
LOCAL_ADDR = "192.168.30.34"
LOCAL_PORT = 15000
BUFF_SIZE = 1024
BYTE_ARRAY_SIZE = 42
HEADER_SIZE = 14
...
def stop_wait_sync(my_socket):
    ...
    while True:
        ...
        my_socket.sendto(str(index + 1).encode(), addr_con)
...
def stop_and_wait_receive(my_socket, filename, filesize):
    ...
    with open(filename, 'wb') as f:
        while filesize > 0:
            # bytes_to_write = filesize % PAYLOAD_SIZE
            ...
            if int((header[:CRC_SIZE]).decode()) == current_crc:
                ...
                my_socket.sendto(str(packet_num - 1).encode(), addr)

    return md5_hash(filename)


def process_file(filename, filesize, my_socket):
    ...
    if sender_hash != str(my_hashfile):
        ...
        while NOT_OK:
            ...
            while True:
                try:
                    my_socket.sendto(b"NOTOK", addr_con)
    ...
    if my_hashfile == sender_hash:
        ...
        while True:
            try:
                my_socket.sendto(b"OK", addr_con)
...
if __name__ == '__main__':
    ...
    process_file(filename, filesize, my_socket)
